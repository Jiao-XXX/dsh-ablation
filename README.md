...
> ## ⏸ 本项目已搁置（2026-08-17）
>
> **不再继续开发。** 判断是：消融与效果验证这件事，DeepSeek 官方大概率会自己解决——由掌握 harness 的一方来做，比外部逆向拆解更直接也更权威。与其维护一套注定被取代的工具，不如把已经得到的结论留在这里。
>
> **仓库保持完整可运行**：`bootstrap.sh` + `bin/doctor.py` 能在任意机器上重建实验环境，11 个 overlay 与 4 个任务集都可直接跑。三轮实验的**原始运行数据一并提交在 `results/`**，报告里的每个数字都能回溯到具体某一次运行。
>
> 如果你要做类似的事，最值得拿走的可能不是代码，而是[报告](docs/REPORT.md)里那三个被推翻的假设，以及第 5 节关于小样本如何稳定产出假象的计算。
